...
public class Character2DController : MonoBehaviour
{
    ...
    /// <summary>
    /// this should be called anytime you have to modify the CapsuleCollider2D at runtime. It will recalculate the distance between the rays used for collision detection.
    ...
    void moveHorizontally( ref Vector3 deltaMovement )
    {
        ...
        for( var i = 0; i < totalHorizontalRays; i++ )
        {
            ...
            {
                if (isGoingRight)
                { deltaMovement.x = _unstuckRaycastHit.point.x - capsuleCollider.bounds.max.x; }
                else
                { deltaMovement.x = _unstuckRaycastHit.point.x - capsuleCollider.bounds.min.x; }
    ...
    void moveVertically( ref Vector3 deltaMovement )
    {
        ...
        for( var i = 0; i < totalVerticalRays; i++ )
        {
            ...
            {
                if (isGoingUp)
                { deltaMovement.y = _unstuckRaycastHit.point.y - capsuleCollider.bounds.max.y; }
                else
                { deltaMovement.y = _unstuckRaycastHit.point.y - capsuleCollider.bounds.min.y; }
    ...
    /// <summary>
    /// checks the center point under the CapsuleCollider2D for a slope. If it finds one then the deltaMovement is adjusted so that
